...
class BaseAgent(ABC) :
    ...
    def get_unit_vector_to_goal(self):
        ...
        if dist < 1e-6:
            return self.u.copy() # 이미 도착했다면 현재 방향 유지
        return vec / dist
    ...
    def get_observation(self, frame_index):
        ...
        # 1. 화면 가시 범위 설정 (목표 지점에 맞춘 동적 스케일링)
        # 목표 지점(x_goal)보다 약 10% 더 여유 있게 범위를 설정하여 '벽 붙음' 방지
        # 최소 범위를 130m로 두어 너무 작은 맵이 되는 것도 방지
        max_x = max(self.env.x_goal[0] * 1.1, 130.0)
        max_z = max(self.env.x_goal[2] * 1.5, 110.0) # 고도는 초기 위치나 목표 중 높은 쪽 고려

...
- 기존 v3 학습/테스트 CSV는 현재 특징 계약과 호환되지 않는다. 새 출력은 `*_features_v4.csv`와 `raw_trajectories_v4.jsonl`로 구분한다.
- 다른 simulator version의 manifest가 있는 폴더는 덮어쓰지 않는다. 특징 수식과 근거는 `FEATURES_V4.md`를 참고한다.
...
현재 bbox는 여전히 투영 실루엣 근사와 A-like ROI 관측 모델이다. 날개 자세·가림·motion blur를 렌더링하고 A 추적기에 실제 통과시킨 결과가 아니다. Feature v4는 이 bbox를 분류 특징에서 제외한다. 다만 중심점에도 합성 고유 패턴과 관측 모델 편향이 남을 수 있다.
